                ...
                <form action="{{ route('admin.games.update', $game->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('release_year') is-invalid @enderror" id="release_year" name="release_year" value="{{ old('release_year', $game->release_year) }}" min="1900" max="{{ date('Y') + 5 }}" required>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('developer') is-invalid @enderror" id="developer" name="developer" value="{{ old('developer', $game->developer) }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('platform') is-invalid @enderror" id="platform" name="platform" value="{{ old('platform', $game->platform) }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('age_rating') is-invalid @enderror" id="age_rating" name="age_rating">
                            <option value="0+" {{ old('age_rating', $game->age_rating ?? '0+') == '0+' ? 'selected' : '' }}>0+ — Для всех возрастов</option>
                            <option value="6+" {{ old('age_rating', $game->age_rating ?? '0+') == '6+' ? 'selected' : '' }}>6+ — Для детей от 6 лет</option>
                            <option value="12+" {{ old('age_rating', $game->age_rating ?? '0+') == '12+' ? 'selected' : '' }}>12+ — Для детей от 12 лет</option>
                            <option value="16+" {{ old('age_rating', $game->age_rating ?? '0+') == '16+' ? 'selected' : '' }}>16+ — Для подростков от 16 лет</option>
                            <option value="18+" {{ old('age_rating', $game->age_rating ?? '0+') == '18+' ? 'selected' : '' }}>18+ — Только для взрослых</option>
                        </select>
                        @error('age_rating')
                    ...
                    <div class="mb-3">
                        ...
                        @if($game->cover_image)
                            <div class="mb-2">
                                <p>Текущая обложка:</p>
                                <img src="{{ Storage::url($game->cover_image) }}" alt="{{ $game->title }}" class="img-thumbnail" style="max-height: 200px;">
                            </div>
                            <div class="form-check mb-2">
                                <input class="form-check-input" type="checkbox" id="remove_cover" name="remove_cover" value="1">
                                <label class="form-check-label" for="remove_cover">Удалить текущую обложку</label>
                            </div>
                        @endif
