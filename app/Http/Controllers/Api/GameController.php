...
class GameController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $cacheKey = 'games_index_' . md5(http_build_query($request->all()));

        $games = Cache::remember($cacheKey, now()->addMinutes(10), function () use ($request) {
        ...
        $genres = Cache::remember('all_genres', now()->addHours(24), function () {
    ...
    public function show($slug)
    {
        ...
        // YouTube трейлер с кешем на 7 дней
        try {
            $cacheKey = 'youtube_trailer_' . md5($game->title . $game->release_year);
            $trailer = Cache::remember($cacheKey, now()->addDays(7), function () use ($game) {
                $youtubeService = new YouTubeService();
                return $youtubeService->searchTrailer($game->title, $game->release_year);
            });
        } catch (\Exception $e) {
            $trailer = null;
        }
    ...
    public function top(Request $request)
    {
        ...
        $cacheKey = 'games_top_page_' . $page;

        $games = Cache::remember($cacheKey, now()->addHours(6), function () {
    ...
    public function newReleases(Request $request)
    {
        ...
        $page = $request->get('page', 1);
        $cacheKey = 'games_new_page_' . $page;

        $games = Cache::remember($cacheKey, now()->addHours(6), function () use ($twoYearsAgo) {
            return Game::with('genre')
    ...
    {
        $cacheKey = 'games_calendar';

        $gamesByDecade = Cache::remember($cacheKey, now()->addHours(24), function () {
            return Game::with('genre')
                ->orderBy('release_year', 'desc')
                ->get()
    ...
    public function store(Request $request)
    {
        ...
        // Очищаем кеш после добавления игры
        Cache::flush();
    ...
    public function update(Request $request, $id)
    {
        ...
        // Очищаем кеш после обновления игры
        Cache::flush();
    ...
    public function destroy($id)
    {
        ...
        // Очищаем кеш после удаления игры
        Cache::flush();
